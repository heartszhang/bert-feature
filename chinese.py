...
def convert_example_to_features(
    uid: int, text_a: str, seq_length: int, tokenizer: BertTokenizer
) -> InputFeatures:
...
def each_line(
    max_seq_len: int, batch_n: int, sep: str, tokenizer
) -> Iterator[List[InputFeatures]]:
    batch = []
    for line in stdin:
...
def main(
    bert_model: str = "/repo/bert-models/ch-base",
    sep: str = ",",
    layer: int = -2,
    max_seq_length: int = 128,
    batch_n: int = 64,
    without_cuda: bool = False,
) -> None:
  ...
  --without-cuda: Whether not to use CUDA.  """
    ...
    for features in each_line(max_seq_length, batch_n, sep, tokenizer):
        ...
        for input_ids, input_mask, uids in eval_dataloader:
            ...
            pools, _ = model(input_ids, token_type_ids=None, attention_mask=input_mask)
            pools = pools[layer]
